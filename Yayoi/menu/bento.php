        ...
        <div class="row bodyMenu">
            <div class="col-lg-9 col-12">
                <div id="foodType" class="text-center">
                    ...
                    $typeTH = array("ทั้งหมด", "เนื้อไก่", "เนื้อปลา");
                    $typeEN = array("all", "chicken", "fish");

                    for ($i = 0; $i < sizeof($typeTH); $i++) {
                        echo "<div class='typeBox' id='" . $typeEN[$i] . "Type' onclick='changeMenuType(\"bento\", \"" . $typeEN[$i] . "\", 1)' onmouseover='changeTypeIcon(\"" . $typeEN[$i] . "\", \"over\")' onmouseout='changeTypeIcon(\"" . $typeEN[$i] . "\", \"out\")'>";
                ...
                <div id="menuFood" class="row">
                    ...
                    for ($i = 0; $i < sizeOf($data); $i++) {
                        if ($i < 15) {
                            ...
                            echo "<button onclick='clickMenu(\"" . $menu->id . "\", \"bento\")' class='btnNone clickCursor'><img src='" . $menu->img . "' width='85%' title='" . $menu->id . " " . $menu->nameTH . "'></button><br><br>";
                            ...
                            echo "</div><button onclick='clickMenu(\"" . $menu->id . "\", \"bento\")' class='linkMenu btnNone clickCursor'><div class='nameMenu font-weight-bold' id='nameMenu$i' onmouseover='changeMenuName($i, \"over\")' onmouseout='changeMenuName($i, \"out\")'>" . $menu->nameTH . "<br>";
                            ...
                            echo "<button class='cartBt' onclick='add(\"" . $menu->id . "\", \"bento\")'><img src='../icon/addcart.PNG' width='70%'></button></div></div>";
                            echo "</div>";
                        } else if ($i == 15) {
                            echo "<div class='pageBar'><button class='pageNum'>‹</button>";
                            for ($j = 1; $j < (sizeof($data) / 15) + 1; $j++) {
                                if ($j == 1) {
                                    echo "<button onclick='changeMenuType(\"bento\", \"all\", $j)' class='pageNum pageSelect' id='page" . ($j) . "'>" . ($j) . "</button>";
                                } else {
                                    echo "<button onclick='changeMenuType(\"bento\", \"all\", $j)' class='pageNum' id='page" . ($j) . "'>" . ($j) . "</button>";
                                }
                            }
